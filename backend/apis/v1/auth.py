...
from backend.schemas.users import RegisterIn, RegisterOut, UserOut
...
def register(register_data: RegisterIn, db: Session = Depends(get_db)):
    ...
    user_db = get_user_by_username(db, register_data.username)

    if user_db:
        raise ServiceException(code="ERR_002", message="用户已存在")

    hashed_password = get_password_hash(register_data.password1)
    user_db = create_user(db, register_data.username, hashed_password)

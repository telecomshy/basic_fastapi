...
from backend.core.dependencies import current_user, session
from fastapi import Depends
from sqlalchemy.orm import Session
from backend.db.models.users import User


def override_current_user(db_session: Session = Depends(session)) -> User:
    user = get_user_by_username(db_session, 'test_user1')
    return user
